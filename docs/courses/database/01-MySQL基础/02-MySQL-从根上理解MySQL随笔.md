...
你想，B树如果**非叶子节点和叶子节点都是记录**的话，那么每一层的记录都需要进行IO读取，磁盘I/O的次数也就越多，而B+树里面非叶子节点存储的**索引更加多，一次读取的数据多，所以I/O次数少**，这只是其一；

其二，如果非叶子节点上存储了记录，那么代表树的深度也会随之增加，相应的查询的性能也会有所下降；
...
连接层，**server层和存储引擎层**，本来查询的数据要在server层中进行过滤，但是现在相当于直接将扫描的条件下推到存储层

下面来自于deepseek的回答:

```bash
SELECT serial_number, COUNT(id) 
FROM coupon 
WHERE serial_number IN ('xxx') AND status IN (1,2,4,5)
GROUP BY serial_number;
1. ICP 的完整工作流程
首先通过索引快速定位 serial_number IN ('xxx') 的所有记录，获取它们的 主键ID（聚簇索引键）。
ICP 的优化尝试：
虽然 status 不在 idx_serial_number 中，但存储引擎会利用主键ID 的某些特性（如哈希值或物理位置）进行粗粒度过滤。
例如：如果某些主键ID 的范围明显不可能包含 status=1，则提前排除。
注意：这种过滤是启发式的，不保证完全准确，目的是减少回表量。
2.为什么 ICP 能利用主键ID 优化？
主键ID 与数据的物理关联：
InnoDB 中，主键ID 直接关联到聚簇索引的物理存储位置，而同一 serial_number 的主键ID 可能集中在某些数据页中。如果这些页的元信息（如 MIN/MAX 统计值）显示不可能包含目标 status，则可跳过。

统计信息辅助：
MySQL 会收集索引的统计信息（如直方图），帮助判断主键ID 与 status 的潜在关联性。

局限性：
这种优化是概率性的，可能减少回表次数（如从 10 次降到 5 次），但无法像覆盖索引那样彻底避免回表
```

总结："ICP 尝试优化" 的本质是存储引擎利用主键ID 和统计信息进行概率性过滤，属于尽力而为的优化。

它不能替代覆盖索引，但能在一定程度上减少回表操作。
...
原子性
隔离性
一致性 符合现实世界的约束(书上是这么说的,我还没有看其他的，根据Guide的说法，理解为转账操作前的总额是不变的)
持久性
AID是手段，C才是我们的目标
...
首先我们要知道解决各个隔离级别下的并发的问题有两种思路，一种是MVCC，另一种就是锁的方案，但是锁的方案会导致并发的性能下降。所以，这个时候MVCC无疑是更加轻量级的选择，但是不是说有了他就可以不用锁，因为它只解决了并发读的问题，但是，如果要并发写入的话，还是需要锁来进行控制。
